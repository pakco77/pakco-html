# pakco.html — *Visual taste menu for AI-generated decks*
...
pakco.html is a fork/packaging layer. The real visual work belongs to the upstream authors:
...
pakco.html is a local-first *visual taste menu* for AI agents. It does not try to be an online PPT editor — it solves a different problem: **how do I tell an agent my taste in a stable, repeatable way?** Browse real previews, click a card, get an executable prompt for Claude Code / Codex / Hermes. The agent generates the deck (or skin, template, social-card set, UI) following the visual constraints you picked.

## ▶ 30-second start

1. **Install** the skill (one line — see [Install](#-install)).
2. **Open the picker.** It ships at `templates/style-picker.html` inside the installed skill. Either open it yourself in a browser, or just tell your agent *"open the pakco.html picker"*. For live iframe previews, serve the folder (see Install).
...
**Not a fit**
- You want a WYSIWYG online PPT editor with drag-and-drop. pakco.html is a *menu + prompt launcher*, not an editor.
...
```
~/.claude/skills/pakco-html/      # Claude Code
~/.codex/skills/pakco-html/       # Codex
~/.hermes/skills/pakco-html/      # Hermes Agent
~/.cursor/…  /  others          # any AgentSkill-aware agent
```

The picker is the same file in every case: `…/pakco-html/templates/style-picker.html`. Copying a prompt into an agent needs **no MCP**. MCP/a local bridge is only needed if you want the web page to directly launch an agent, read local files, or write the generated folder.
...
open ~/.claude/skills/pakco-html/templates/style-picker.html
...
```
pakco-html/
├── SKILL.md                      agent-facing dispatcher (skill name: pakco-html)
...
├── templates/
│   ├── style-picker.html         ★ the pakco.html WebUI (Skins/Templates/UI Taste/Social Cards/Guide)
...
└── examples/                     local builds + pakco-html.local.json (git-ignored)
```
...
- You can stack **many** custom skins/templates — each appends to `examples/pakco-html.local.json`, never overwrites, and shows as a `local` card after you refresh the picker.
- Manual path: `./scripts/new-deck.sh <slug>` then `open examples/<slug>/index.html`.

`pakco-html.local.json` is git-ignored — it's your personal taste library, not a public catalog, and it does not change the public counts.
...
- **Menu, not generator.** pakco.html copies executable prompts; your agent does the generating. That keeps it model-agnostic and zero-build.
